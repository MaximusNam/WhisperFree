...
# Сколько ждать, пока человек отпустит клавиши сочетания. Это НЕ то же
# ожидание, что перед вставкой: там человек уже отпустил клавишу диктовки, и
# 400 мс хватает с запасом. Здесь обработчик стартует по НАЖАТИЮ, то есть в
# тот момент, когда Ctrl и Alt заведомо зажаты, — и пока они зажаты, послать
# копирование нельзя: окно получит Ctrl+Alt+C, а это не копирование.
HOLD_RELEASE_MS = 3000


def copy_selection(
    combo: str = "ctrl+c", timeout_ms: int = 600, wait_modifiers_ms: int = HOLD_RELEASE_MS
) -> tuple[str | None, str]:
    """Текст, выделенный в активном окне, и причина неудачи.
    ...
    Возвращает (текст, причина). Текст пустой, а причина непустая — не
    получилось, и причину надо показать человеку: «ничего не выделено» и
    «вы держите Ctrl» требуют от него совершенно разных действий.
    ...
    # Ждём отпускания и ПРОВЕРЯЕМ результат. Раньше здесь результат не
    # проверялся, и по истечении ожидания копирование уходило поверх зажатых
    # клавиш: окно получало Ctrl+Alt+C, буфер не менялся, а человек читал на
    # плашке «выделите исправленный текст» — при том, что текст он выделил.
    if not wait_modifiers_released(wait_modifiers_ms):
        return None, "отпустите Ctrl и Alt"
    ...
    if not set_clipboard_text(_PROBE):
        ...
        return None, "буфер обмена занят другим приложением"

    if not send_combo(combo):
        _restore(previous)
        return None, "не удалось отправить сочетание клавиш"
    ...
    if selection is None:
        log.info("после %s буфер не изменился — выделения нет", combo)
        return None, "выделите исправленный текст"
    return selection, ""
