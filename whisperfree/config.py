...
#
# Особое значение "auto" — язык не задавать, пусть определит сам. Но для
# СМЕШАННОЙ речи это не решение, и вот почему: Whisper определяет язык ОДИН
# раз, по началу записи, и дальше расшифровывает этим языком всё остальное.
# Переключаться посреди фразы он не умеет. Поэтому "auto" не убирает выбор
# языка, а превращает его в монетку: услышит украинское начало — и вся
# русская часть уедет в украинский.
#
# Замер на одной и той же записи (21 слово, русские и украинские вперемешку):
#   ru + смешанная затравка   19    <- лучшее и предсказуемое
#   auto + трёхъязычная       19    <- столько же, но язык выбирает монетка
#   ru + русская затравка     17
#   ru без затравки           17
#   uk + украинская затравка  15
#
# Отсюда вывод: для смешанной речи решает не язык, а ЗАТРАВКА. Оставьте здесь
# свой основной язык, а слова второго добавьте в prompt_ru ниже.
main = "ru"
alt = "en"
...
#
# Тот же приём работает и без "auto": если вы говорите по-русски с
# украинскими или английскими вставками, допишите такие вставки прямо в
# prompt_ru — по замеру это дало +2 слова из 21 и оказалось лучше, чем
# менять язык.
