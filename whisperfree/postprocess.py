...
HALLUCINATION_PATTERNS = tuple(
    ...
    for pattern in (
        ...
        # То же по-украински: Whisper учился и на украинских субтитрах,
        # и на тишине выдаёт титры оттуда.
        r"субтитри\s+(?:зробив|створив|підготував|та переклад).*",
        r"редактор\s+субтитрів.*",
        r"продовження\s+далі[.!…\s]*",
        r"підписуйтесь\s+на\s+канал[.!…\s]*",
        r"дякую\s+за\s+(?:перегляд|увагу)[.!…\s]*",
        r"(?:став(?:те)?\s+)?лайки?\s+(?:та|і)\s+підпис\w+.*",
